...
## 2026-08-26 — multi-sheet editor integration

The project model was already sound; the missing layer was editor ownership. The least invasive integration keeps the existing schematic operations intact while changing their source of truth: `project` now holds a normalized `KipadProject`, `sch()` resolves `Project.activeSheet(project).schematic` at each call site, and `setSch()` replaces only that active sheet model. Schematic undo/redo snapshots and restores the complete project JSON, including `activeSheetId`, so edits and sheet switches cannot restore content into the wrong visible sheet.

- Added the schematic sheet strip above the canvas. It renders one named tab per sheet, highlights the active tab, switches on click, creates with `+` and a prompted `Sheet N` default, renames by right-click or long-press, and deletes with an `×` after confirming non-empty sheets. Switching clears transient selection/wire state, recomputes ERC, refreshes Properties, updates the canvas, and reports `Project: <name> — Sheet: <name>` in the status bar. The schematic View menu exposes the same New/Rename/Delete operations.
- Added project file interchange through the existing file input. `.kicad_proj` uses `KipadProject.serializeProject` / `parseProject` behind the same `KipadSafeSave` round-trip gate as schematic saving; `.kicad_sch` still parses through the existing schematic parser and is wrapped with `Project.fromSchematic`. Normal Save preserves `.kicad_sch` for one-sheet projects, while multi-sheet projects and the explicit **Save Project (.kicad_proj)** action use project JSON and the `kipad.backup.proj.v1` backup ring. The existing `kipad.backup.sch.v1` path remains intact.
- Extended `kipad.board.v1` localStorage payloads with a normalized `project` alongside `board`; old payloads without that field retain the previous lazy one-sheet startup path.
- Added `test/test_project_integration.js` for the real source wiring, active-sheet getter/setter behavior, whole-project undo snapshots, sheet-safe restore, SafeSave-stable project JSON, `.kicad_sch` wrapping, and localStorage compatibility. All **47/47** dependency-free Node suites pass; `node --check` passes for every app part and `project.js`; `git diff --check` is clean. The browser-shell smoke was attempted, but Chromium again did not expose its DevTools endpoint in this environment.
- Cache discipline: changed the edited shell references to `?v=61`, advanced the service-worker cache from `kipad-v56` to `kipad-v57`, and verified `./js/project.js` remains in the precache asset list.

This slice deliberately stops at sheet ownership/navigation and project file/session persistence. Hierarchical/global-label connectivity, further project export refinement, and cross-sheet ERC conflict checks remain separate unchecked milestone sub-items.
