...
assert.deepStrictEqual(loaded.sheets.map(s => s.name), ['Main', 'Power']);
assert.strictEqual(Project.activeSheet(loaded).name, 'Power');
assert.strictEqual(loaded.sheets[0].schematic.paper, 'A3');
assert.strictEqual(loaded.sheets[0].schematic.labels[0].text, 'INPUT');
assert.strictEqual(loaded.sheets[1].schematic.labels[0].text, 'VCC');
